...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    if (strncmp(argv[i], "--phase=", 8) == 0)
      ...
      phase = atoi(argv[++i]);
    else if (strncmp(argv[i], "--svd_n_cut=", 11) == 0)
      svd_n_cut = atoi(argv[i] + 11);
  ...
  if (phase == 1)
  {
    ...
    b_4_zero = true;
    svd_n_cut = 2;
    double phase_i_scl_ksi[7] = {0e0, 1e2, 5e0, 5e0, 5e0, 5e0, 5e0};
    for (int i = 0; i < 7; ++i)
      scl_ksi[i] = phase_i_scl_ksi[i];
    double phase_i_scl_a[4] = {1e0, 1e0, 1e0, 1e0};
    for (int i = 0; i < 4; ++i)
      scl_a[i] = phase_i_scl_a[i];
    double phase_i_scl_K_avg[5] = {1e-3, 1e-3, 1e-3, 5e3, 5e3};
    for (int i = 0; i < 5; ++i)
