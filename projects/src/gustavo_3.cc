...
const double
    ...
    bnL_scl[] = {0e0, 0e0, 0e0, 1e0, 5e1 / 1e2, 5 * 1e4}, // Scaling of multipole {_, b_1, b_2, ...}
...
public:
  ...
  void set_Jacob(const int k, double val)
  {
    ...
    {
      throw std::out_of_range("set_Jacob: index " + std::to_string(k) +
...
Lie_gen_class get_Lie_gen(const std::string label, const tps &g, const double scl, const int i,
...
void prt_Lie_gen(const std::string &str, int &i0, const int n,
...
Lie_gen_class get_Lie_K_avg_gen(const std::string label, const tps &K, const double scl,
...
void prt_Lie_K_avg_gen(const std::string &str, int &i0, const int n,
                       const std::vector<Lie_gen_class> &Lie_gen)
{
  printf("\n%s\n", str.c_str());
  for (int k = i0; k < i0 + n; k++)
...
std::vector<Lie_gen_class> get_K_avg(const tps &K)
{
  ...
  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[0], 2, 2, 0, 0, 0, 4, 4, 0, 0, 0));

  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[1], 0, 0, 2, 2, 0, 0, 0, 4, 4, 0));

  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[2], 1, 1, 1, 1, 0, 2, 2, 1, 1, 0));
  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[2], 1, 1, 1, 1, 0, 1, 1, 2, 2, 0));

  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[3], 1, 1, 0, 0, 2, 1, 1, 0, 0, 4));
  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[3], 1, 1, 0, 0, 3, 1, 1, 0, 0, 5));

  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[4], 0, 0, 1, 1, 2, 0, 0, 1, 1, 4));
  K_avg_2.push_back(get_Lie_K_avg_gen("<K>", K, scl_K_avg[4], 0, 0, 1, 1, 3, 0, 0, 1, 1, 5));
...
void compute_Jacob(const param_type &bns, const int prm_no, const ss_vect<tps> &Id_scl,
...
inline void compute_Jacob(const ss_vect<tps> &Id_scl, const param_type &bns,
                          std::vector<Lie_gen_class> &Lie_gen)
{
...
void get_Lie_gen(std::vector<Lie_gen_class> &Lie_gens,
                 const std::vector<Lie_gen_class> &Lie_gen)
{
...
std::vector<Lie_gen_class> compute_Lie_gen(const ss_vect<tps> &Id_scl, const param_type &bns)
{
  double nu[3], xi[2];
...
void get_system(const int m, const int n, const std::vector<Lie_gen_class> &Lie_gen,
                double **A, double *b)
{
...
void correct(param_type &bns, const std::vector<Lie_gen_class> &Lie_gen,
...
void get_bns(param_type &bns)
{
  ...
  case 51:
    if (b_3_opt)
    {
      ...
      bns.add_Fam("o1_n1", Sext, bnL_min[Sext], bnL_max[Sext], bnL_scl[Sext]);
      bns.add_Fam("o2_n1", Sext, bnL_min[Sext], bnL_max[Sext], bnL_scl[Sext]);
      bns.add_Fam("o3_n1", Sext, bnL_min[Sext], bnL_max[Sext], bnL_scl[Sext]);
    }
  ...
  case 53:
    ...
    if (b_4_opt)
    {
      bns.add_Fam("o1_n1_sl", Oct, bnL_min[Oct], bnL_max[Oct], bnL_scl[Oct]);
      bns.add_Fam("o2_n1_sl", Oct, bnL_min[Oct], bnL_max[Oct], bnL_scl[Oct]);
      bns.add_Fam("o3_n1_sl", Oct, bnL_min[Oct], bnL_max[Oct], bnL_scl[Oct]);
    }
    break;
  case 6:
    if (b_3_opt)
